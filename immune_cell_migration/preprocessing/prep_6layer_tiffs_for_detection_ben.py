...
def downsample(img, factor):
    ...
    z_down = (h2 / H, w2 / W) if img.ndim == 2 else (h2 / H, w2 / W, 1.0)
    return zoom(img, z_down, order=1, grid_mode=True)
...
def prepare_unet_input(pathlist, rep_spacing=3, downsampling_factor=3):
    ...
    for item in pathlist:
        ...
        for fname in sorted(os.listdir(base_dir)):
            ...
            pos = fname.split('_x00_y00')[0].split('_')[-1]
        ...
        for pos in positions:
            ...
            for fname in sorted(os.listdir(base_dir)):
                if not fname.endswith(".tif") or ("rep" not in fname) or (pos not in fname):
                    continue
            ...
            if len(file_index) == 0:
                print(f"No valid TIFF files found for {pos} in {base_dir}")
                continue
            ...
            for rep_center in available_reps:
                ...
                try:
                    ...
                    out_name = os.path.basename(file_index[rep_t0]["Imin"])
                    out_path = os.path.join(output_dir, out_name)
                    imwrite(out_path, multi_channel, photometric="minisblack")
                    print(f"Saved: {out_path}")

                except Exception as e:
                    print(f"Failed at {pos} rep{rep_center:04d} in {base_dir}: {e}")
                    continue