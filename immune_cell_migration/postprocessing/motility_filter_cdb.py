...
MOTILITY_DEFINITION = {"NK": 6.5, "Jurkat": 4, "NK_day14": 13}

def filter_cdb(celltype, path_list, pixelsize_ccd=4.0954, objective=10, time_step=15, thresh_motile=6.5):
    res = pixelsize_ccd/objective  # Lumenera ; 6.45/10  Hamamatsu
    save_name_csv = '_' + str(thres_motile) + 'umin5min'
...
def get_speed_stepwidth(nanpadded_trackarray):
...
def measure_tracks(nanpadded_trackarray, time_step, res):
    ...
    speed_stepwidth_um_min = get_speed_stepwidth(nanpadded_trackarray)
...
def extract(db, data):
    ...
    nanpadded_trackarray = getTracksNanPadded2(db, type=track_type, layer=1)
    ...
    for id in data.id.unique():
        ...
        try:
            ...
            pass
        try:
            data.loc[data.id == id, "cos_angle"] = adjustLength2(cos_angle[0], 0)
        except ValueError:
            pass


def get_tracks_nan_padded(self, type=None, id=None, start_frame=None, end_frame=None, skip=None, layer=0, apply_offset=False):
    """
...
def database_to_pandas(db):
    ...
    data["motile"] = data["distance_um"] > thres_motile
